...
public class BookinfoController implements Initializable{
	...
	public void clicktoBorrow10days(ActionEvent event) {
		...
		try {
			...
			int stock = 0;
			String getStock = "select inStock from bookinfo where ISBN=?";
			stmt = connectMysql.Connnector.executePreparedStatement(getStock);
			stmt.setString(1, ISBN);
			rs = stmt.executeQuery();
			while(rs.next()) {
				stock = rs.getInt("inStock");
			}
			if(credits<=0||stock==0) {
				Alert alert = new Alert(AlertType.WARNING);
				alert.setContentText("Either your credits is less than 0 so can't use borrow function now, (please go to reception to pay cash"+
				"to ransom your credits) or maybe the stock is empty now.Check your profile and the borrow page to see details.");
				alert.showAndWait();
			}
...
// reserve function to be implemented:check bookID to see whether it is reserved. Then check whether the user is the one who reserved the book
	...
	public void clicktoReserve(ActionEvent event) {
		...
		if(credits<=0) {
			Alert alert = new Alert(AlertType.WARNING);
			alert.setContentText("You don't have enough credits to use reservation function!");
			alert.showAndWait();
		}
